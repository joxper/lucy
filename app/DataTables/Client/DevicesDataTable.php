        ...
        return $this->datatables
            ...
            ->addColumn('action', function ($data) use($permissions) {
                ...
                if (isset($permissions['edit']) && $permissions['edit'] && Guard::hasAccess($permissions['edit'])) {
                    $action .= '<a href="' . action('Modules\DeviceController@edit', ['id' => $data->id]) . '" class="btn btn-icon-only green" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;';
                }

                $action .= '<a href="' . action('Modules\DeviceController@show', ['id' => $data->id]) .'" class="btn btn-icon-only purple" title="View"><i class="fa fa-eye"></i></a>';
