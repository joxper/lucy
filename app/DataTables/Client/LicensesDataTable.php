...
use App\Models\Modules\License;
use Sentinel;
use App\Lucy\Guard;
        ...
        return $this->datatables
            ...
            ->addColumn('action', function ($data) use($permissions) {
                ...
                if (isset($permissions['edit']) && $permissions['edit'] && Guard::hasAccess($permissions['edit'])) {
                    $action .= '<a href="' . action('Modules\LicenseController@edit', ['id' => $data->id]) . '" class="btn btn-icon-only green" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;';
                }

                $action .= '<a href="' . action('Modules\LicenseController@show', ['id' => $data->id]) .'" class="btn btn-icon-only purple" title="View"><i class="fa fa-eye"></i></a>';
